 ...
 * @brief Builds and maintains a global elevation map using Bayesian fusion of transformed LiDAR pointclouds.
 *
 * This node implements a global mapping module that fuses 3D pointcloud data into a 2D grid-based elevation map.
 * It uses a per-cell recursive Bayesian update (via the BayesFilter class) to smooth noisy elevation readings.
 * An optional filter pass improves terrain continuity by enforcing maximum gradient thresholds between neighbors.
 *
 * The node listens to pointclouds already transformed into the map frame and maintains both raw and filtered
 * elevation occupancy grids, which are published and saved for further use in navigation or analysis.
 *
 * @version 1.0.0
 * @date 2025-07-27
 *
 ...
 *
 * Subscribers:
 * - /mapping/transformed_pointcloud: [sensor_msgs::msg::PointCloud2] LiDAR pointcloud transformed into the map frame.
 *
 * Publishers:
 * - /mapping/global_map: [nav_msgs::msg::OccupancyGrid] Raw elevation map (currently commented out).
 * - /mapping/filtered_global_map: [nav_msgs::msg::OccupancyGrid] Smoothed and filtered elevation map.
 *
 * Parameters (Constants/macros used):
 * - MAP_RESOLUTION: Grid resolution (meters per cell).
 * - MAP_DIMENSION: Total size of the map (meters).
 * - ELEVATION_SCALE: Factor to convert z-values into occupancy cell values.
 * - MAPPING_SOURCE_DIR: File system path for map storage.
 *
 * Main Components:
 * - `setupCommunications()`: Initializes ROS publishers, subscribers, timers, and TF listener.
 * - `configureMaps()`: Initializes the global and filtered maps, loads an existing map from PCD if available.
 * - `transformedPCLCallback()`: Starts a detached thread for map fusion when new pointcloud data is received.
 * - `fuseMap()`: Aggregates elevation values into cells, averages them, and applies Bayes filtering.
 * - `filterMap()`: Applies spatial smoothing to enforce a max gradient constraint on elevation differences.
 * - `transformMap()`: Converts pointclouds from base_link frame to map frame using TF2 and Eigen.
 * - `saveGlobalMapCSV()`: Exports the filtered elevation map as a CSV file for offline analysis or visualization.
 * - `publishGlobalMap()`: Publishes the latest filtered map.
 *
 * Mapping Model:
 * - Per-cell Kalman-like update:
 *   \f[
 *   z_t = \hat{z}_{t-1} + K_t(z_{meas} - \hat{z}_{t-1}), \quad K_t = \frac{\sigma^2_{t-1}}{\sigma^2_{t-1} + \sigma^2_t}
 *   \f]
 * - Neighbor updates are applied when gradients exceed `ELEVATION_SCALE / 0.866` (derived from 30° slope).
 *
 * Assumptions:
 * - Pointclouds are pre-registered to the map frame or can be transformed from base_link to map.
 * - Elevation data is used as a proxy for traversability or terrain modeling.
 *
 * @credit Based on classic Bayesian elevation mapping and grid-based terrain modeling approaches.
 */
