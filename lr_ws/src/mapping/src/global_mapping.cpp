 ...
 * @brief Builds and maintains a global elevation occupancy map from transformed point clouds.
 *
 * This node subscribes to a transformed point cloud, fuses it into a global elevation map using a
 * Bayes filter for smoothing, and publishes both the raw and filtered global maps. The filtered
 * map is updated by comparing elevation differences and adjusting neighboring cells within a set
 * gradient threshold. An initial preload from a saved PCD map is supported.
 *
 * @version 1.0.0
 * @date 2025-08-09
 *
 ...
 *
 * Subscribers:
 * - /mapping/transformed_pointcloud: [sensor_msgs::msg::PointCloud2] Incoming point cloud aligned to the map frame.
 *
 * Publishers:
 * - /mapping/global_map: [nav_msgs::msg::OccupancyGrid] Raw global elevation occupancy grid.
 ...
 *
 * Parameters:
 * - MAP_RESOLUTION: [double] Grid cell size in meters.
 * - MAP_DIMENSION: [double] Overall map width/height in meters.
 * - ELEVATION_SCALE: [double] Scaling factor for elevation values.
 * - LOCALIZATION_VARIANCE (via BayesFilter): [float] Minimum variance for elevation estimation.
 *
 * Notes:
 * - Initializes occupancy grids with all zero values and a Bayes filter per cell.
 * - Preloads an existing map from PCD if available; otherwise starts with empty map.
 * - Incoming point clouds are optionally transformed from `base_link` to `map` using TF lookup.
 * - Elevations are accumulated per cell and averaged based on point density.
 * - `filterMap()`:
 *   - Updates cells whose elevation differs from the filtered map by more than the gradient threshold.
 *   - Adjusts neighboring cells up or down to maintain elevation continuity.
 * - Saves filtered map as a CSV file after the initial fusion step.
 * - Publishes the filtered map at a fixed interval (1 second).
 */
