 ...
 * @brief Builds and maintains a local elevation occupancy map from transformed point clouds in the robot frame.
 *
 * This node subscribes to a point cloud aligned to the `base_link` frame, fuses it into a local elevation
 * occupancy grid using a Bayes filter for smoothing, and publishes both the raw and filtered local maps.
 * The filtered map is updated by comparing elevation differences and adjusting neighboring cells within a
 * defined gradient threshold. The local map is reset and recomputed for each incoming point cloud message.
 *
 * @version 1.0.0
 * @date 2025-08-09
 *
 ...
 *
 * Subscribers:
 * - /mapping/transformed_pointcloud: [sensor_msgs::msg::PointCloud2] Incoming point cloud aligned to `base_link` frame.
 *
 * Publishers:
 * - /mapping/local_map: [nav_msgs::msg::OccupancyGrid] Raw local elevation occupancy grid (optional publish).
 ...
 *
 * Notes:
 * - Local map frame is fixed to `base_link`; origin offset in Y is set to -1.0.
 * - For each new point cloud, the local map is cleared and recalculated.
 * - Elevations are averaged per grid cell based on point density.
 * - `filterMap()`:
 *   - Updates cells exceeding the gradient threshold compared to the filtered map.
 *   - Smooths neighboring cells to maintain elevation continuity.
 * - Access to map data is protected by a mutex during updates and publishing.
 * - Publishes the filtered local map at a fixed 1-second interval.
 */
