/**
 * @file common.cpp
 * @brief Provides utility functions for planning operations, including point/pose creation, distance calculations with Euclidean formula, 
 * and coordinate transformations.
 *
 * This module offers helper functions for handling geometric data, such as creating 2D points and poses, computing Euclidean distances, 
 * converting angle units, transforming points and poses between frames, calculating angle differences, checking pose proximity, and finding the
 * closest trajectory index based on robot state.
 *
 * @version 1.0.0
 * @date 2025-07-19
 *
 * Methods:
 * - create_point2d(): Creates a 2D point from x and y coordinates.
 * - create_pose2d(): Creates a 2D pose from x, y, and yaw.
 * - euclidean_distance(): Calculates distance between two points using √[(x2 - x1)^2 + (y2 - y1)^2].
 * - rad2deg(): Converts radians to degrees using (180 * rad / π).
 * - deg2rad(): Converts degrees to radians using (π * deg / 180).
 * - transformPointGlobalToLocal(): Transforms a point from global to local frame using affine transformations.
 * - transformPoint(): Transforms a point using a pose matrix.
 * - transformPose(): Transforms a pose relative to another using rotation and translation.
 * - smallest_angle_difference_signed(): Computes the signed smallest angle difference using modular arithmetic.
 * - samePoseWithinThresh(): Checks if two poses are within thresholds using Euclidean distance and angle difference.
 * - getClosestTrajIndex(): Finds the closest index in a trajectory based on distance and orientation checks.
 *
 ...
 * @return lr_msgs::msg::Point2D 
 */
lr_msgs::msg::Point2D transformPointGlobalToLocal(const lr_msgs::msg::Point2D &global_pt, const lr_msgs::msg::Pose2D &local_frame) {
...
lr_msgs::msg::Point2D transformPoint(const lr_msgs::msg::Point2D &source_pt, const lr_msgs::msg::Pose2D &pose) {
...
lr_msgs::msg::Pose2D transformPose(
  ...
  lr_msgs::msg::Point2D transformed_point = transformPoint(source_pose.pt, transforming_pose);
...
bool samePoseWithinThresh(const lr_msgs::msg::Pose2D &pose1, const lr_msgs::msg::Pose2D &pose2, const float thresh_pos, const double thresh_head) {
