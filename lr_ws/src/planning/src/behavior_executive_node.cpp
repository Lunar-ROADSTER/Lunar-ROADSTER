/**
 * @file behavior_executive_node.cpp
 * @brief Orchestrates the behavior executive node for planning, managing FSM states, navigation, visualization, and tool control in a ROS2 environment.
 *
 * This node initializes publishers for visualization and commands, subscribers for robot state and debug triggers, service and action clients for navigation 
 * and worksystem operations, and timers for FSM and visualization updates. It loads map parameters, handles FSM state transitions, processes robot odometry, 
 * controls tool trajectories, publishes visualization markers, and manages navigation goals to ensure coordinated mission execution.
 *
 * Publishers:
 * - /viz/planning/current_goals: [geometry_msgs::msg::PoseArray] Array of current goal poses for visualization.
 * - /viz/planning/all_state_l1_goals: [geometry_msgs::msg::PoseArray] Array of state L1 goal poses for visualization.
 * - /viz/planning/current_agent: [geometry_msgs::msg::PoseStamped] Current agent pose for visualization.
 * - /viz/planning/current_goal: [geometry_msgs::msg::PoseStamped] Current goal pose for visualization.
 * - /autonomy_cmd: [lr_msgs::msg::ActuatorCommand] Commands for wheel velocity, steer position, and tool height.
 * - /tool_height: [std_msgs::msg::Float64] Tool height value.
 * - /publish_local_map: [std_msgs::msg::Bool] Command to publish local map.
 *
 * Subscribers:
 * - /odometry/filtered/ekf_global_node: [nav_msgs::msg::Odometry] Incoming global robot state data.
 * - /odometry/filtered/ekf_odom_node: [nav_msgs::msg::Odometry] Incoming odometry robot state data.
 * - /behavior_exec_debug_trigger: [std_msgs::msg::Bool] Debug trigger signals.
 *
 * Services:
 * - /enable_worksystem_server: [lr_msgs::srv::EnableWorksystem] Client for enabling worksystem operations.
 *
 * Parameters:
 * - fsm_timer_callback_ms: [int] FSM timer interval in milliseconds.
 * - service_response_timeout_ms: [int] Service response timeout in milliseconds.
 * - viz_timer_callback_ms: [int] Visualization timer interval in milliseconds.
 * - height, width, resolution: [int, int, float] Map dimensions and resolution.
 * - design_topo_filepath, site_topo_filepath: [std::string] File paths for design and site topology maps.
 * - xTransform, yTransform: [float] Map transformation offsets.
 * - source_threshold_z, sink_threshold_z: [float] Thresholds for source and sink in transport planning.
 * - boundary_min, boundary_max: [float] Boundaries for planning.
 * - thresh_max_assignment_distance: [float] Maximum distance threshold for assignments.
 * - topology_equality_threshold: [float] Threshold for map similarity checks.
 * - viz_planning_height: [double] Height for visualization markers.
 *
 * @version 1.0.0
 * @date 2025-07-19
 *
 ...
 *
 * @credit John Harrington, Team CraterGrader. Based on ROS2 architecture and FSM structure from the project's codebase.
 */
...
BehaviorExecutive::BehaviorExecutive() : Node("behavior_executive_node")
{
  ...
  cmd_pub_ = this->create_publisher<lr_msgs::msg::ActuatorCommand>("/autonomy_cmd", 1);
  ...
  // site_map_client_ = this->create_client<lr_msgs::srv::SiteMap>("site_map_server", rmw_qos_profile_services_default, site_map_client_group_);
  // update_trajectory_client_ = this->create_client<lr_msgs::srv::UpdateTrajectory>("update_trajectory_server", rmw_qos_profile_services_default, update_trajectory_client_group_);
  enable_worksystem_client_ = this->create_client<lr_msgs::srv::EnableWorksystem>("enable_worksystem_server", rmw_qos_profile_services_default, enable_worksystem_client_group_);
...
void BehaviorExecutive::fsmTimerCallback()
{
  ...
  {
  case lr::planning::FSM::StateL0::READY:
    ready_.runState();
    break;

  case lr::planning::FSM::StateL0::UPDATE_MAP:
    map_updated_ = updateMap(false);
    // Check that map was updated correctly
    update_map_.runState(map_updated_, traj_debug_);
    break;

  case lr::planning::FSM::StateL0::SITE_WORK_DONE:
  // TODO: FVD Task
  // Implement validation stack
    site_work_done_.runState();
    break;

  case lr::planning::FSM::StateL0::MAP_EXPLORED:
    map_explored_.runState();
    break;

  case lr::planning::FSM::StateL0::PLAN_TRANSPORT:
  ...
  case lr::planning::FSM::StateL0::GET_TRANSPORT_GOALS:
    num_poses_before_ = current_goal_poses_.size();
  ...
  case lr::planning::FSM::StateL0::FOLLOWING_TRAJECTORY:
  {
  ...
  case lr::planning::FSM::StateL0::END_MISSION:
    end_mission_.runState();
    break;

  case lr::planning::FSM::StateL0::STOPPED:
    stopped_.runState();
    break;
...
void BehaviorExecutive::globalRobotStateCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
  ...
  current_agent_pose_ = lr::planning::create_pose2d(global_robot_state_.pose.pose.position.x, global_robot_state_.pose.pose.position.y,  global_robot_yaw);
  // current_agent_pose_ = lr::planning::transformPose(global_robot_pose_, global_map_relative_to_local_frame_);
...
void BehaviorExecutive::vizTimerCallback() {
  ...
  // Agent
  if (init_robot_pose_) {
    lr_msgs::msg::Pose2D global_agent_pose = lr::planning::transformPose(current_agent_pose_, local_map_relative_to_global_frame_);
  ...
  // Current goal
  lr_msgs::msg::Pose2D global_curr_goal_pose = lr::planning::transformPose(current_goal_pose_, local_map_relative_to_global_frame_);
  ...
  for (lr_msgs::msg::Pose2D goal_pose : current_goal_poses_) {
    lr_msgs::msg::Pose2D global_goal_pose = lr::planning::transformPose(goal_pose, local_map_relative_to_global_frame_);
  ...
  for (lr_msgs::msg::Pose2D goal_pose : viz_state_l1_goal_poses_) {
    lr_msgs::msg::Pose2D global_goal_pose = lr::planning::transformPose(goal_pose, local_map_relative_to_global_frame_);
...
{
  // if (!wait_for_tool_) {
  lr_msgs::msg::ActuatorCommand cmd_msg;
...
bool BehaviorExecutive::updateMap(bool verbose = false) {
  ...
  lr::planning::generateZeroMapCsv(map_height, map_width, map_resolution);
  lr::planning::generateElevationMapCsv(map_height, map_width, map_resolution);
...
// Action
void BehaviorExecutive::sendNavigationGoal(const lr_msgs::msg::Pose2D & goal_pose_2d)
{
