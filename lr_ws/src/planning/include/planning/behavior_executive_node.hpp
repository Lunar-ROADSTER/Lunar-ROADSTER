...
#include <lr_msgs/srv/site_map.hpp> // Service for receiving SiteMap height data
#include <lr_msgs/srv/update_trajectory.hpp>   // Service for updating current trajectory
#include <lr_msgs/srv/enable_worksystem.hpp> // Service to enable/disable worksystem controller
#include <lr_msgs/msg/actuator_command.hpp> // Publisher for tool height
...
private: 
  ...
  rclcpp::Client<lr_msgs::srv::SiteMap>::SharedPtr site_map_client_;
  rclcpp::Client<lr_msgs::srv::UpdateTrajectory>::SharedPtr update_trajectory_client_;
  rclcpp::Client<lr_msgs::srv::EnableWorksystem>::SharedPtr enable_worksystem_client_;
  ...
  void sendNavigationGoal(const lr_msgs::msg::Pose2D & goal_pose_2d);
  ...
  lr_msgs::msg::Pose2D global_robot_pose_;
  lr_msgs::msg::Pose2D current_agent_pose_;
  ...
  std::vector<lr_msgs::msg::Pose2D> current_goal_poses_;
  std::vector<std::string> current_goalPose_types;
  lr_msgs::msg::Pose2D current_goal_pose_;
  ...
  /* Variables */
  lr::mapping::Map<float> current_height_map_;
  lr::mapping::Map<float> design_height_map_;
  lr::mapping::Map<float> seen_map_;

  float current_map_coverage_ratio_;
  std::vector<int> current_seen_map_;
  lr_msgs::msg::Pose2D local_map_relative_to_global_frame_;
  lr_msgs::msg::Pose2D global_map_relative_to_local_frame_;
  ...
  std::vector<lr_msgs::msg::Pose2D> viz_state_l1_goal_poses_;

  /* Transport planner */
  std::unique_ptr<lr::planning::TransportPlanner> transport_planner_;
  size_t num_poses_before_ = 0;

  /* FSM */
  lr::planning::FSM fsm_;

  // FSM states
  lr::planning::Ready ready_;
  lr::planning::UpdateMap update_map_;
  lr::planning::SiteWorkDone site_work_done_;
  lr::planning::MapExplored map_explored_;
  lr::planning::PlanTransport plan_transport_;
  lr::planning::GetTransportGoals get_transport_goals_;
  lr::planning::GoalsRemaining goals_remaining_;
  lr::planning::GetWorksystemTrajectory get_worksystem_trajectory_;
  lr::planning::FollowingTrajectory following_trajectory_;
  lr::planning::EndMission end_mission_;
  lr::planning::Stopped stopped_;
