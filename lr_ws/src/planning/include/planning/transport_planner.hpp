...
public:
  ...
  // Computations()
  float planTransport(const lr::mapping::Map<float> &current_height_map, const lr::mapping::Map<float> &design_height_map, const std::vector<int> &seen_map, const float thresh_max_assignment_distance, std::vector<std::string> &goalPoses_types);
  std::vector<lr_msgs::msg::Pose2D> getGoalPose(const lr_msgs::msg::Pose2D &agent_pose, const lr::mapping::Map<float> &map, std::vector<std::string> &goalPose_types);
  std::vector<lr_msgs::msg::Pose2D> getUnvisitedGoalPoses(std::vector<std::string> &goalPose_types);
  void getGoalPoseType(std::vector<std::string> &goalPose_types);
  void makeGoalsFromAssignment(const std::vector<TransportAssignment> &transport_assignments, const size_t assignment_idx, std::vector<lr_msgs::msg::Pose2D> &goalPoses, std::vector<std::string> &goalPose_types);
  ...
  void init_nodes(std::vector<TransportNode> &source_nodes, std::vector<TransportNode> &sink_nodes, float &vol_source, float &vol_sink, const lr::mapping::Map<float> &current_height_map, const lr::mapping::Map<float> &design_height_map, const std::vector<int> &seen_map);
