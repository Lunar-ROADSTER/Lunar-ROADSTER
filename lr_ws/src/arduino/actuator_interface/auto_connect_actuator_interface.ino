...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time)
{
    ...
    if (timer != NULL)
    {
        if (cmd_msg_received)
        {
            ...
            if (abs(drive_cmd_raw - 127) <= 1)
                drive_cmd_raw = 127; // Fix to zero when close to zero
            ...
            if (abs(steer_cmd_raw - 127) <= 1)
                steer_cmd_raw = 127; // Fix to zero when close to zero
            ...
            if (abs(tool_cmd_raw) <= 1)
                tool_cmd_raw = 0; // Fix to zero when close to zero
