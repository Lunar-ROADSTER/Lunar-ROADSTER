 ...
 *
 * Subscribers:
 * - /encoder_telemetry: [lr_msgs::msg::EncoderTelemetry] Raw telemetry from drive and steer encoders.
 *
 * Publishers:
 * - /encoder/odom: [nav_msgs::msg::Odometry] Robot odometry in the `odom` frame.
 * - /actuator/state: [lr_msgs::msg::ActuatorState] Computed actuator state including velocity, steer angle, and tool position.
 *
 * Services:
 * - None
 * 
...
OdomNode::OdomNode() : Node("odom_node") {
  ...
  act_state_pub_ = this->create_publisher<lr_msgs::msg::ActuatorState>(
    "/actuator/state", 1
  );
  enc_sub_ = this->create_subscription<lr_msgs::msg::EncoderTelemetry>(
