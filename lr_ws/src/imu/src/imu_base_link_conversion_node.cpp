 ...
 * @brief Converts IMU messages from the imu_link frame to base_link using TF2.
 *
 * This node transforms IMU data received in the `imu_link` frame into the `base_link` frame by applying a static transform
 * lookup between the two frames. The transformation is applied to orientation, angular velocity, and linear acceleration,
 * ensuring the resulting message is suitable for use in localization and control stacks expecting base_link-aligned data.
 *
 * The node listens to `/vectornav/imu` (typically the raw IMU topic) and publishes the transformed message to `/imu/data/base_link`.
 * Covariances are preserved from the original message. Orientation is transformed using quaternion rotation, and angular
 * velocity and linear acceleration vectors are rotated using Eigen.
 *
 * @version 1.0.0
 * @date 2025-07-27
 *
 * Maintainer: Bhaswanth Ayapilla, Boxiang (William) Fu, Ankit Aggarwal
 * Team: Lunar ROADSTER
 * Team Members: Ankit Aggarwal, Deepam Ameria, Bhaswanth Ayapilla, Simson D’Souza, Boxiang (William) Fu
 *
 * Subscribers:
 * - /vectornav/imu: [sensor_msgs::msg::Imu] Raw IMU measurements in the imu_link frame.
 *
 * Publishers:
 * - /imu/data/base_link: [sensor_msgs::msg::Imu] Transformed IMU measurements in the base_link frame.
 *
 * Parameters: None
 *
 * Transform:
 * - A static transform is retrieved from TF2 between `base_link` and `imu_link` at node startup.
 * - All IMU fields are transformed:
 *    - Orientation is transformed using: `q_out = r * q_in * r⁻¹`
 *    - Angular velocity and linear acceleration are rotated using `R * v_in`
 *    - Covariances are passed through without frame change, assuming symmetric sensors
 *
 * Assumptions:
 * - Transform from `imu_link` to `base_link` is static and available at startup.
 * - Sensor placement and orientation are correct and aligned with TF configuration.
 *
 * @credit Transform logic based on ROS1 `imu_transformer` and TF2 best practices, adapted for ROS2 with Eigen integration.
 */
