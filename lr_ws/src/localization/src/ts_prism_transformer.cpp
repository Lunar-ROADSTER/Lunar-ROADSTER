/**
 * @file ts_prism_transformer.cpp
 * @brief Transforms total station prism measurements into base_link poses and provides IMU yaw calibration through an action server.
 *
 * This node fuses raw prism position measurements from a total station with IMU orientation data to estimate the 6-DOF transform
 * of the robot base (`base_link`) in the map frame. A fixed transform between the prism and base_link is used to compute the final position,
 * and the yaw is optionally corrected using an offset obtained via IMU calibration.
 *
 * The calibration action computes a yaw offset between the IMU orientation and bearing changes inferred from TS position deltas.
 * This offset is applied to correct IMU yaw drift or misalignment. The node also provides TF publishing and listening capabilities.
 *
 * @version 1.0.0
 * @date 2025-07-07
 *
 ...
 *
 * Subscribers:
 * - /total_station_prism: [geometry_msgs::msg::PoseWithCovarianceStamped] Raw prism pose in map frame.
 * - /imu/data/base_link: [sensor_msgs::msg::Imu] IMU orientation for estimating robot heading.
 *
 * Publishers:
 * - /transformed_total_station_prism: [geometry_msgs::msg::PoseWithCovarianceStamped] Transformed pose of base_link.
 * - TF: [geometry_msgs::msg::TransformStamped] Optional transform updates (map → base_link, base_link → prism).
 *
 * Action Servers:
 * - ts_prism_transformer/calibrate_imu: [CalibrateImu] Records TS+IMU data over time to compute yaw calibration offset.
 *
 * Parameters:
 * - Hardcoded static transform: (x, y, z) from prism to base_link in robot frame.
 * - pub_freq: Frequency of TF updates (default: 10 Hz).
 *
 * Features:
 * - Applies a static transform from prism to base_link.
 * - Uses tf2 to listen and broadcast transforms.
 * - Maintains a yaw offset based on IMU-prism calibration procedure.
 *
