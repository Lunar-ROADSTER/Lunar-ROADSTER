 ...
 * @brief Teleoperation interface node for mapping joystick input to actuator commands.
 *
 * This ROS2 node translates joystick (`sensor_msgs::msg::Joy`) input into actuator command messages
 * (`lr_msgs::msg::ActuatorCommand`) that control wheel velocity, steering angle, and tool height.
 * It enables manual control of the rover during teleoperation and autograding modes, while also 
 * responding to mode switches via the multiplexer mode topic (`lr_msgs::msg::MuxMode`).
 *
 * Key functionalities:
 * - Maps joystick axes/buttons to actuator command values.
 * - Publishes actuator command only in `FULL_TELEOP` or `AUTOGRADER` mux modes.
 * - Subscribes to actuator commands to retain state when direct teleoperation is not active.
 * - Responds to mux mode changes to allow or restrict manual control.
 * - Uses a timer to publish commands periodically at 10 Hz.
 *
 * Joystick controls:
 * - Drive forward/backward mapped to a configurable axis (e.g., left stick vertical).
 * - Steer left/right mapped to another axis (e.g., left stick horizontal).
 * - Tool up/down adjusted incrementally using bumper buttons (when in FULL_TELEOP mode).
 *
 * @version 1.0.0
 * @date 2025-07-29
 *
 * Maintainer: Bhaswanth Ayapilla, Deepam Ameria  
 * Team: Lunar ROADSTER  
 * Team Members: Ankit Aggarwal, Deepam Ameria, Bhaswanth Ayapilla, Simson D’Souza, Boxiang (William) Fu
 *
 * Subscribers:
 * - /joy: [sensor_msgs::msg::Joy] Joystick input from teleop user.
 * - /actuator_cmd: [lr_msgs::msg::ActuatorCommand] Feedback loop to retain command state outside of teleop.
 * - /mux_mode: [lr_msgs::msg::MuxMode] Determines which mode the robot is in, to control publishing logic.
 *
 * Publishers:
 * - /teleop_cmd: [lr_msgs::msg::ActuatorCommand] Command values mapped from joystick.
 *
 * Parameters:
 * - axis_drive: Index of joystick axis for driving (forward/backward).
 * - axis_steer: Index of joystick axis for steering (left/right).
 * - tool_raise: Button index to raise tool.
 * - tool_lower: Button index to lower tool.
 * - tool_increment: Not currently used, reserved for future fine-grained control.
 *
...
TeleopNode::TeleopNode() : Node("teleop_node") {
  ...
  act_cmd_sub_ = this->create_subscription<lr_msgs::msg::ActuatorCommand>(
...
void TeleopNode::timerCallback() {
  auto cmd_msg = lr_msgs::msg::ActuatorCommand();
  ...
  if (curr_mux_mode_ == lr_msgs::msg::MuxMode::FULL_TELEOP || curr_mux_mode_ == lr_msgs::msg::MuxMode::AUTOGRADER)
  {
...
void TeleopNode::modeCallback(const lr_msgs::msg::MuxMode::SharedPtr msg)
{
...
void TeleopNode::actCmdCallback(const lr_msgs::msg::ActuatorCommand::SharedPtr msg)
{
  ...
  if (curr_mux_mode_ != lr_msgs::msg::MuxMode::FULL_TELEOP) {
...
void TeleopNode::joyCallback(const sensor_msgs::msg::Joy::SharedPtr msg) {
  ...
  if (curr_mux_mode_ == lr_msgs::msg::MuxMode::FULL_TELEOP) {
    ...
    if (msg->buttons[joy_lower_tool_i_]) {
      ...
      joy_tool_height_state_ = std::max(0.0, joy_tool_height_state_ - 10.0);
    }    
    //If raise bumper pressed, then raise by 5%
    if (msg->buttons[joy_raise_tool_i_]) {
      joy_tool_pressed_ = true;  
      joy_tool_height_state_ = std::min(100.0, joy_tool_height_state_ + 10.0);
    }
