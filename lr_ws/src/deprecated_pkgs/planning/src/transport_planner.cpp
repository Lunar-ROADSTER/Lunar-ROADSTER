/**
 * @file transport_planner.cpp
 * @brief Implements transport planning using optimization to assign sources to sinks, generate goal poses, and filter assignments based on distances and thresholds.
 *
 * This module uses OR-Tools for linear programming to solve transport problems, initializing nodes from height maps, calculating distances with Euclidean formula √[(x2 - x1)^2 + (y2 - y1)^2], and generating poses for navigation. It handles source and sink nodes, filters assignments to ensure proximity thresholds, and supports a toy problem for verification.
 *
 * Data Structures:
 * - TransportNode: Represents a node with x, y coordinates and height.
 * - TransportAssignment: Represents an assignment with source node, sink node, and transport volume.
 *
 * @version 1.0.0
 * @date 2025-07-19
 *
 * Parameters:
 * - source_threshold_z [float]: Threshold for identifying source nodes.
 * - sink_threshold_z [float]: Threshold for identifying sink nodes.
 * - boundary_min [float]: Minimum boundary for pose validation.
 * - boundary_max [float]: Maximum boundary for pose validation.
 * - thresh_filter_assignment_pos [float]: Position threshold for filtering assignments.
 * - thresh_filter_assignment_head [double]: Heading threshold for filtering assignments.
 * - thresh_max_assignment_distance [float]: Maximum distance threshold for valid assignments.
 * - source_pose_offset [double]: Offset for source poses.
 * - last_pose_offset [double]: Offset for the last pose.
 * - boundary_increment [float]: Increment for boundary adjustments.
 * - sink_pose_offset [float]: Offset for sink poses.
 * - new_sink_offset [float]: Offset for new sink poses.
 * - last_pose_offset_constrained [double]: Constrained offset for the last pose.
 *
 * Methods:
 * - ij_to_index(): Converts row and column to a single index.
 * - makeGoalsFromAssignment(): Generates goal poses from assignments, using yaw from atan2((sink_y - source_y) / (sink_x - source_x)).
 * - getGoalPose(): Selects the closest unvisited goal pose based on Euclidean distance.
 * - getUnvisitedGoalPoses(): Retrieves poses for unvisited assignments.
 * - getGoalPoseType(): Sets types for goal poses (e.g., 'offset', 'source').
 * - init_nodes(): Initializes source and sink nodes from maps, calculating volumes.
 * - calculate_distances(): Computes distances between nodes using Euclidean formula.
 * - solveForTransportAssignments(): Solves optimization using OR-Tools to minimize total distance, subject to volume constraints.
 * - filterAssignments(): Filters assignments based on pose proximity thresholds.
 * - planTransport(): Orchestrates the full transport planning process.
 * - solveToyProblem(): Tests the solver with a predefined problem.
 *
 * Maintainer: Boxiang (William) Fu
 * Team: Lunar ROADSTER  
 * Team Members: Ankit Aggarwal, Deepam Ameria, Bhaswanth Ayapilla, Simson D’Souza, Boxiang (William) Fu
 *
 * @credit John Harrington, Team CraterGrader. Based on OR-Tools optimization and ROS2 planning utilities in the project.
 */
...
void TransportPlanner::makeGoalsFromAssignment(const std::vector<TransportAssignment> &transport_assignments, const size_t assignment_idx, std::vector<lr_msgs::msg::Pose2D> &goalPoses, std::vector<std::string> &goalPose_types)
{
  ...
  // Set up sink pose
  lr_msgs::msg::Pose2D sink_pose = lr::planning::create_pose2d(transport_assignments[assignment_idx].sink_node.x, transport_assignments[assignment_idx].sink_node.y, yaw);

  // Set up source pose
  lr_msgs::msg::Pose2D source_pose = lr::planning::create_pose2d(transport_assignments[assignment_idx].source_node.x, transport_assignments[assignment_idx].source_node.y, yaw);
  ...
  source_pose = lr::planning::create_pose2d(source_offset_pose_x, source_offset_pose_y, yaw);
  ...
  sink_pose = lr::planning::create_pose2d(sink_offset_pose_x, sink_offset_pose_y, yaw);

   //Calculate distance between source and sink 
  // double manipulation_distance = lr::planning::euclidean_distance(source_pose.pt, sink_pose.pt);
  ...
  lr_msgs::msg::Pose2D sink_backblading = lr::planning::create_pose2d(new_sink_x, new_sink_y, yaw); // TODO: Check if this is correct



  // Create final poses
  lr_msgs::msg::Pose2D offset_pose = lr::planning::create_pose2d(offset_pose_x, offset_pose_y, yaw);
 ...
 * @return lr_msgs::msg::Pose2D The pose to navigate to 
 */
std::vector<lr_msgs::msg::Pose2D> TransportPlanner::getGoalPose(const lr_msgs::msg::Pose2D &agent_pose, const lr::mapping::Map<float> &map, std::vector<std::string> &goalPose_types){
  ...
  for (size_t i = 0; i < transport_assignments_.size(); ++i){
    ...
    if (unvisited_assignments_.at(i)){
      ...
      lr_msgs::msg::Point2D src_point = lr::planning::create_point2d(transport_assignments_.at(i).source_node.x, transport_assignments_.at(i).source_node.y);
      // calculate distance between pose and transport assignment
      float dist = lr::planning::euclidean_distance(agent_pose.pt, src_point);
...
std::vector<lr_msgs::msg::Pose2D> TransportPlanner::getUnvisitedGoalPoses(std::vector<std::string> &goalPose_types){
  std::vector<lr_msgs::msg::Pose2D> unvisitedGoalPoses;
 ...
 */
void TransportPlanner::init_nodes(std::vector<TransportNode> &source_nodes, std::vector<TransportNode> &sink_nodes, float &vol_source, float &vol_sink, const lr::mapping::Map<float> &current_height_map, const lr::mapping::Map<float> &design_height_map, const std::vector<int> &seen_map)
{
  ...
  for (size_t i = 0; i < num_cells; i++)
  {
    ...
    lr_msgs::msg::Point2D pt = current_height_map.indexToContinuousCoords(i);
...
void TransportPlanner::calculate_distances(std::vector<float> &distances_between_nodes, const std::vector<TransportNode> &source_nodes, const std::vector<TransportNode> &sink_nodes)
{
  ...
  for (size_t i = 0; i < source_nodes.size(); i++)
  {
    for (size_t j = 0; j < sink_nodes.size(); j++)
    {
      ...
      lr_msgs::msg::Point2D source_pt = create_point2d(source_nodes[i].x, source_nodes[i].y);
      lr_msgs::msg::Point2D sink_pt = create_point2d(sink_nodes[j].x, sink_nodes[j].y);
...
float TransportPlanner::solveForTransportAssignments(std::vector<TransportAssignment> &new_transport_assignments, const std::vector<TransportNode> &source_nodes, const std::vector<TransportNode> &sink_nodes, const std::vector<float> &distances_between_nodes, const float vol_source, const float vol_sink, const float thresh_max_assignment_distance, std::vector<std::string> &goalPose_types ,bool verbose = false)
{
  ...
  for (size_t i = 0; i < n_policy; i++)
  {
    for (size_t j = 0; j < m_policy; j++)
    {
      ...
      lr_msgs::msg::Point2D source_pt = create_point2d(source_nodes[i].x, source_nodes[i].y);
      lr_msgs::msg::Point2D sink_pt = create_point2d(sink_nodes[j].x, sink_nodes[j].y);
      if ((transport_volume > 0) && (lr::planning::euclidean_distance(source_pt, sink_pt) <= thresh_max_assignment_distance))
      {
...
void TransportPlanner::filterAssignments(std::vector<TransportAssignment> &new_transport_assignments, std::vector<std::string> &goalPose_types) {
  ...
  for (size_t i=0; i < new_transport_assignments.size(); ++i) {
    ...
    std::vector<lr_msgs::msg::Pose2D> assignment_goal_poses;
    makeGoalsFromAssignment(new_transport_assignments, i, assignment_goal_poses, goalPose_types);
    lr_msgs::msg::Pose2D assignment_source_pose = assignment_goal_poses[0];
    bool no_similar_poses = true;

    for (size_t j=0; j < i; ++j) {
      
      std::vector<lr_msgs::msg::Pose2D> check_assignment_goal_poses;
      makeGoalsFromAssignment(new_transport_assignments, j, check_assignment_goal_poses, goalPose_types);
      lr_msgs::msg::Pose2D check_assignment_source_pose = check_assignment_goal_poses[0];

      // Skip transport that is too close
      if (lr::planning::samePoseWithinThresh(assignment_source_pose, check_assignment_source_pose, thresh_filter_assignment_pos_, thresh_filter_assignment_head_) && keep_assignment[j]) {
  ...
  // lr_msgs::msg::Point2D crater_center = lr::planning::create_point2d(4.066517f, 3.01634f);
  // // lr_msgs::msg::Point2D crater_center = lr::planning::create_point2d(4.3f, 2.8f);


  // // Check for the one that is closest to crater center 
  // for(size_t j = 1; j < filtered_transport_assignments.size(); ++j) {
  //   lr_msgs::msg::Point2D sink_pt = create_point2d(filtered_transport_assignments[j].source_node.x, filtered_transport_assignments[j].source_node.y);
  ...
  //   // sink_pt = lr::planning::create_point2d(sink_offset_pose_x, sink_offset_pose_y);

  //   float best_distance = std::numeric_limits<float>::max();
  //   if (lr::planning::euclidean_distance(sink_pt, crater_center) < best_distance) {
  //     best_distance = lr::planning::euclidean_distance(sink_pt, crater_center);
 ...
 */
float TransportPlanner::planTransport(const lr::mapping::Map<float> &current_height_map, const lr::mapping::Map<float> &design_height_map, const std::vector<int> &seen_map, const float thresh_max_assignment_distance, std::vector<std::string> &goalPoses_types)
{
  // ---------------------------------------------------
